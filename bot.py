...
(
    MAIN_MENU,
    ADD_RECIPIENT_NAME,
    ADD_RECIPIENT_RELATION,
    ADD_RECIPIENT_CONTACT,
    ADD_RECIPIENT_TONE,
    ADD_RECIPIENT_SCHEDULE,
    ADD_RECIPIENT_SCHEDULE_TIME,
    SET_CITY,
    SET_SEND_MODE,
    PREVIEW_CONFIRM,
) = range(10)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "Ти один раз налаштовуєш — я роблю все інше.\n\n"
...
async def show_recipients(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not recipients:
        ...
        for r in recipients:
            ...
            schedule = r.get('schedule_days', 'не задано')
            schedule_time = r.get('schedule_time', '')
            text += (
                f"{relation_label} — *{r['name']}*\n"
                f"   Тон: {tone_label}\n"
                f"   Розклад: {schedule} о {schedule_time}\n\n"
            )
            buttons.append([InlineKeyboardButton(
                f"🗑 Видалити {r['name']}", callback_data=f"del_{r['id']}"
            )])
...
async def add_recipient_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [[InlineKeyboardButton(label, callback_data=f"rel_{key}")]
...
async def add_recipient_relation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "Введи їх Telegram username (наприклад @mama_ukraine) "
        "або номер телефону (+380XXXXXXXXX):",
        parse_mode="Markdown"
    )
...
async def add_recipient_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [[InlineKeyboardButton(label, callback_data=f"tone_{key}")]
...
async def add_recipient_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "days_done":
        ...
        days_str = ", ".join([DAYS_UK.get(d, d) for d in context.user_data['rec_days']])
        if 'everyday' in context.user_data['rec_days']:
            days_str = "Щодня"
        await query.edit_message_text(
            f"⏰ О котрій годині надсилати?\n"
            f"Дні: *{days_str}*\n\n"
            f"Введи час у форматі HH:MM (наприклад 09:00):",
            parse_mode="Markdown"
        )
    ...
    day_buttons.append([InlineKeyboardButton(
        "✅ Щодня" if 'everyday' in selected else "Щодня", callback_data="day_everyday"
    )])
...
async def add_recipient_schedule_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    days_str = "Щодня" if 'everyday' in context.user_data['rec_days'] else ", ".join(
        [DAYS_UK.get(d, d) for d in context.user_data['rec_days']]
    )

    await update.message.reply_text(
        f"✅ *Одержувача додано!*\n\n"
        f"👤 {relation_label} — *{context.user_data['rec_name']}*\n"
        f"🎭 Тон: {tone_label}\n"
        f"📅 Розклад: {days_str} о {time_text}\n\n"
        f"Я буду автоматично надсилати повідомлення за розкладом 💌",
        parse_mode="Markdown",
...
async def ask_city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.edit_message_text(
        "🌍 Введи своє місто (наприклад: Київ, Берлін, Варшава):\n"
        "Це потрібно для показу погоди у повідомленнях."
    )
    return SET_CITY
...
async def save_city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Тепер у повідомленнях буде актуальна погода 🌤",
        parse_mode="Markdown",
        reply_markup=main_keyboard()
    )
    return MAIN_MENU
...
async def show_send_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "⚙️ *Режим надсилання*\n\nОбери, як я маю надсилати повідомлення:",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
...
async def save_send_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    labels = {
        "auto": "🤖 Авто — буду надсилати сам",
        "preview": "👁 Буду показувати тобі перед надсиланням",
        "manual": "✏️ Буду чекати твого підтвердження"
    }
    await query.edit_message_text(
...
async def send_now_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not recipients:
        await query.edit_message_text(
            "❌ Немає одержувачів. Спочатку додай хоча б одного.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ Додати", callback_data="add_recipient")]
            ])
        )
    ...
    for r in recipients:
        buttons.append([InlineKeyboardButton(
            f"📨 Надіслати {RELATIONS.get(r['relation'], '')} {r['name']}",
            callback_data=f"sendtest_{r['id']}"
        )])
    buttons.append([InlineKeyboardButton("◀️ Назад", callback_data="back_main")])
...
async def send_test_to(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if send_mode == 'preview':
        ...
        context.user_data['preview_recipient'] = recipient
        await query.edit_message_text(
            f"👁 *Попередній перегляд:*\n\n{message_text}\n\n"
            f"Надіслати *{recipient['name']}*?",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
        ...
        await query.edit_message_text(f"📨 Надсилаю *{recipient['name']}*...", parse_mode="Markdown")
        success = await deliver_message(context, recipient, message_text)
        if success:
            await query.edit_message_text(
                f"✅ Повідомлення надіслано *{recipient['name']}* 💌\n\n_{message_text}_",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("◀️ Головне меню", callback_data="back_main")]
                ])
            ...
            await query.edit_message_text(
                f"⚠️ Не вдалося надіслати. Перевір контакт одержувача.\n\nПовідомлення було:\n_{message_text}_",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("◀️ Головне меню", callback_data="back_main")]
                ])
...
async def confirm_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    success = await deliver_message(context, recipient, message_text)
    result = "✅ Надіслано!" if success else "⚠️ Помилка надсилання"
    await query.edit_message_text(
        f"{result}\n\n_{message_text}_",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
...
async def deliver_message(context, recipient, text):
    try:
        ...
        if contact.startswith('@'):
            ...
            logger.info(f"SMS delivery to {contact} not yet configured")
            return False
        return True
...
# ─────────────────────────────────────────────
def main():
    token = os.environ.get("BOT_TOKEN", "8858159789:AAGsJ34iDA0v3eU8l-It5ZnWX_ZxYFUj65c")
